...
int packet_tap(int tap_socket, int af_socket, int iol_ifid) {
    ...
    if ((length = tap_receive(&eth_frame, tap_socket)) <= 0) {
        ...
        if ((write(af_socket, iol_frame, length + 8)) < 0) {
            rc = 3;  // Was (before 'stop on reload' issue fix)
            //rc = 0;    // Sooner or later writing to af_socket fails if the node is reloaded. Just ignore this error & continue to work.
